...
public static class StudentService
{
    ...
    public static List<Student> LoadFromJson(string filepath)
    {
        try
        {
            ...
            {
                Console.WriteLine($"学生文件不存在。创建一个新的。");
            ...
            Console.WriteLine($"成功加载 {students.Count} 名学生");
            return students;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"加载学生数据时出错: {ex.Message}");
    ...
    /// 获取活跃的学生列表
    /// </summary>
    /// <param name="students">所有学生列表</param>
    /// <returns>活跃学生列表</returns>
